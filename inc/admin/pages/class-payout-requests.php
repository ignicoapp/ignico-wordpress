...
class Payout_Requests {
	 ...
	 */
	public function run() {
		$this->plugin['loader']->add_action( 'current_screen', $this, 'redirect_if_not_configured_and_authorized' );
	...
	/**
	 * Redirect user to settings page if 
	 */
	public function redirect_if_not_configured_and_authorized() {

		$screen = get_current_screen();

		if( ( $screen instanceof \WP_Screen ) && PostType::ID === $screen->post_type ) {
			if(
				! $this->plugin['admin/settings']->is_configured() ||
				! $this->plugin['admin/settings']->is_authorized()
			) {
				wp_safe_redirect( $this->plugin['admin/pages']->get_admin_plugin_url( 'ignico', 'authorization' ) );
				exit;
			}
		}
	}
